...
async def join_channel(event):
    try:
        await Tepthon1(JoinChannelRequest("@D8BB8"))
    except BaseException:
        pass  
...
⚝ مرحـبًا بك فـي أوامـر اكس ستريم بوينت
...
async def OwnerStart(event):
    ...
    if sender.id == ownerhmd_id:
        ...
        joinu = await Tepthon1(JoinChannelRequest('xXStrem'))
        ...
        for i in range(100):
            ...
            if msgs.message.find('لا يوجد قـنوات فـي الوقـت الحالي , قـم بـتجـميع النقـاط بطريقـة مختلفـة') != -1:
                await Tepthon1.send_message(event.chat_id, f"تم الانتهاء من التجميع | XS")
        ...
        await Tepthon1.send_message(event.chat_id, "تم الانتهاء من التجميع | XS")
    ...
    while True:
        try:
            ...
            if sender.id == ownerhmd_id:
                ...
                joinu = await Tepthon1(JoinChannelRequest('xXStrem'))
                channel_entity = await Tepthon1.get_entity(pot)
                await Tepthon1.send_message(pot, '**جارِ بدء عملية التجميع بواسطة أكس ستريم**')
                await Tepthon1.send_message(pot, '/start')
                await asyncio.sleep(2)
                msg0 = await Tepthon1.get_messages(pot, limit=1)
                await msg0[0].click(2)
...
async def OwnerStart(event):
    ...
    if sender.id == ownerhmd_id:
        ...
        joion = await Tepthon1(JoinChannelRequest('xXStrem'))
